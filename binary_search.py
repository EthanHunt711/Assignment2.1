def binary_search(l_sorted, target_x):
    ...
    try:
        ...
        if right >= left:
            ...
            if target_x == l_sorted[middle]:  # conditioning the left and right bound collide
                return True
            elif target_x > l_sorted[middle]:  # if the target value is in the left half
                left = middle+1  # alter the right bound
                return binary_search(l_sorted[middle+1:], target_x)
            else:  # if the target value is in the right half
                right = middle-1  # alter the left bound
